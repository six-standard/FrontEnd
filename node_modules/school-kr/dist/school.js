...
var School = /*#__PURE__*/function () {
  ...
  _createClass(School, [{
    ...
    key: "_parseCalendar",
    value: function _parseCalendar(ev, sep) {
      if (ev) {
        var res = ''; // 하루에 2개 이상의 일정이 있는 경우 | 로 구분되어있음
    ...
    value: function getMeal(year, month) {
      ...
      return this._requestManager.post(this._mealUrl, {
        ...
        data.resultSVO.mthDietList.forEach(function (meal) {
          mealData.push(_this._parseMeal(meal.sun));
          mealData.push(_this._parseMeal(meal.mon));
          mealData.push(_this._parseMeal(meal.tue));
          mealData.push(_this._parseMeal(meal.wed));
          mealData.push(_this._parseMeal(meal.the));
          mealData.push(_this._parseMeal(meal.fri));
          mealData.push(_this._parseMeal(meal.sat));
        });
        var res = {};
    ...
    value: function getCalendar(year, month) {
      ...
      var schulKndScCode = '0' + schulCrseScCode; // 새학기일 경우 2월달 이전의 달에 대한 학사 일정을 가져오는 예외처리

      if (new Date().getFullYear() === year && month <= 2) {
        return this._requestManager.post(this._calendarYearUrl, {
          ay: _util["default"].paddingNumber(year - 1),
          // 이전 년도 데이터를 사용
          schulCode: this._schoolCode,
          schulKndScCode: schulKndScCode,
          schulCrseScCode: schulCrseScCode,
          ...
          var calendarData = [];
          data.resultSVO.selectYear.forEach(function (calendar, i) {
            // day1, event1 과 같은 형식의 프로퍼티에 날짜와 학사일정 데이터가 존재함
            var event = _this2._parseCalendar(calendar['event' + (4 + month)], option.separator || ',');
