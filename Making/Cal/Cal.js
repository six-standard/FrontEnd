...
for(let i = 0; i<=41; i++) { days[i] = document.querySelector("#d" + i); }
switch(date.getMonth()+1) {
    case 2:
        for(let i = 1; i<=28; i++) { days[i-1+firstdate].innerText = i; if(date.getDate()==i) days[i-1+firstdate].className="today"; }
      break;
    case 4:
    case 6:
    case 9:
    ...
    for(let i=0; i < cal.length; i++) {
      if(cal[i].includes("행사내용")) {
        events.push(cal[i]
          ...
          .replaceAll("<div class=calendar_last", "")
          .replaceAll(">"+date.getFullYear()+"년 "+(Number(month)+2)+"월", "")
          );
      }
    }
    let now_events = events[month-2].split(">");
    for(let i = 0; i<now_events.length; i++) {
      tmp += '<'+now_events[i];
    }
    now_events = tmp.replaceAll("<<", "<").split("<");
    for(let i = 0; i<now_events.length; i++) {
      if(now_events[i].includes("일:")) {
